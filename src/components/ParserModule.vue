  ...
  <NCard title="句型判定与推导生成" embedded>
    <NSpace vertical :size="16">
      ...
      <template v-if="derivationResult">
        ...
        <NCardNested
          v-if="derivationResult.reductions && derivationResult.handles"
          title="基于句柄的最左归约流程"
          embedded
        >
          <NSpace vertical :size="8">
            <div
              v-for="(reduction, idx) in derivationResult.reductions"
              :key="`r-${idx}`"
              style="font-family: monospace; padding: 4px 0"
            >
              ...
              <span>{{ reduction }}</span>
              <NTag
                v-if="derivationResult.handles[idx]"
                size="small"
                type="warning"
