...
export const qaRules: QARule[] = [
  ...
  },
  {
    ...
    course: 'compiler',
    question: '什么是句柄？',
    answer: '句柄是右句型中匹配产生式右部的子串，且将其归约到产生式左部的非终结符代表了最右推导的逆过程的一步。即最左归约的每一步匹配的串就是句柄。'
  },
  {
    id: 1002,
    ...
    answer: '0型(短语结构文法)：由图灵机识别。1型(上下文有关文法)：产生式左侧长度不超过右侧，由线性有界自动机识别。2型(上下文无关文法)：产生式左侧只有一个非终结符，由下推自动机识别。3型(正则文法)：左侧单非终结符，右侧为统一的单侧线性(左线性或右线性)，由有限自动机识别。'
  },
  {
    id: 1003,
    ...
    answer: '词法分析是编译器的第一个阶段，它的任务是从左到右逐个字符地扫描源程序，识别出一个个单词(Token)。常见的专业计算机词法单元分为：关键字、标识符、常数(常量)、运算符、界符等。'
  }
]
